...
namespace BHDUserHilari.Test.Tests
{
    public class UsersServiceTests
    {
        ...
        public async Task RegisterAsync_Should_Return_Error_When_Email_Invalid()
        {
            ...
            Assert.Equal("Formato de correo inválido", result.Error);
        }
        ...
        public async Task RegisterAsync_Should_Return_Error_When_Password_Invalid()
        {
            ...
            Assert.True(result.HasError);
            Assert.Equal("Formato de contraseña inválido", result.Error);
        }

        [Fact]
        public async Task RegisterAsync_Should_Return_Error_When_Email_Exists()
        {
            _usersRepositoryMock.Setup(repo => repo.GetByEmailAsync("test@email.com"))
            ...
            Assert.True(result.HasError);
            Assert.Equal("El correo ya registrado", result.Error);
        }

        [Fact]
        public async Task RegisterAsync_Should_Succeed_When_Valid()
        {
            _usersRepositoryMock.Setup(repo => repo.GetByEmailAsync(It.IsAny<string>()))
                .ReturnsAsync((Users)null);

            _jwtGeneratorMock.Setup(jwt => jwt.GenerateToken(It.IsAny<Guid>(), It.IsAny<string>()))
